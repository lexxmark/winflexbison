...
//  sigset_t saved_mask;
  ...
  asyncsafe_spin_lock (&element->lock);
  if (!element->closed)
    {
      ret = _close (element->fd);
      saved_errno = errno;
      element->closed = true;
    }
  else
  ...
  asyncsafe_spin_unlock (&element->lock);
  element->done = true;
...
{
  if (_unlink (absolute_file_name) < 0 && cleanup_verbose
      && errno != ENOENT)
    {
      error (0, errno,
...
//  if (mt) gl_lock_lock (file_cleanup_list_lock);
  ...
  if (gl_list_search (file_cleanup_list, absolute_file_name) == NULL)
    {
      char *absolute_file_name_copy = _strdup (absolute_file_name);
