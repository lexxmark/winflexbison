...
#ifdef _WIN32
static __inline void* mempcpy(void* dst, const void* src, size_t len)
{
   return (char*)memcpy(dst, src, len) + len;
}
#define rawmemchr(p, c) memchr(p, c, (size_t)-1)
#define readlink(r, b, s) -1 // TODO
#endif
                                ...
                                struct scratch_buffer *rname_buf)
{
  ...
  if (!IS_ABSOLUTE_FILE_NAME (name))
    {
      while (!_getcwd (rname, rname_buf->length))
        {
          switch (errno)
            {
            case ERANGE:
